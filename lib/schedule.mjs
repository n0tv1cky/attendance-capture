...
// Finds every subject that could plausibly be "the class we just finished /
// are in" right now: today's timetable row, each configured time slot for
// today's weekday/weekend, resolved to a subject code, within
// [slot.start - a bit, slot.end + graceMinutes] of the current time.
export function findCandidateSessions(timetableRows, config, now = new Date()) {
  const header = timetableRows[0];
  ...
  for (const slot of slots) {
    ...
    const startMin = toMinutes(slot.start) - 15; // allow joining/marking a little early
    const endMin = toMinutes(slot.end) + grace;
    if (nowMin >= startMin && nowMin <= endMin) candidates.push(entry);
  }

  return { todayRow, candidates, allToday };
}

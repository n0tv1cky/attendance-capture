...
// The authoritative session number for a subject's occurrence on `upTo`'s
// date: count how many times that subject has appeared in the timetable,
// chronologically, up to and including that date.
//
// This replaced an earlier design that picked the attendance sheet's target
// "Session N" column by fill state (a column with some rows filled and some
// blank = "in progress, use this one"). That heuristic has a real failure
// mode: a past session with one genuine absentee looks *identical* to a
// column someone just started filling in today -- both are "some filled,
// some blank" with no way to tell them apart from cell contents alone.
// Counting the subject's occurrences in the schedule sidesteps this
// entirely, and also sidesteps the original problem that motivated NOT
// parsing a number out of the timetable cell text (see resolveSubjectCode
// above) -- occurrence counting never needs to read a number out of the
// cell, only which subject it resolves to, which is the reliable part.
//
// Assumes at most one occurrence of a given subject per day, which holds
// for every subject in this schedule (checked against the real data).
export function countSessionOccurrences(timetableRows, subjectCode, config, upTo = new Date()) {
  const upToMidnight = new Date(upTo.getFullYear(), upTo.getMonth(), upTo.getDate());
  const dateColIdx = 0;
  const dataRows = timetableRows.slice(1);
  ...
  for (const row of dataRows) {
    const d = parseSheetDate(row[dateColIdx]);
    if (!d || d > upToMidnight) continue;
    const occursToday = row.some((cell, idx) => idx > 1 && resolveSubjectCode(cell, config.subjectCodeMap, config.nonClassMarkers) === subjectCode);
    if (occursToday) count++;
  }
  return count;
}

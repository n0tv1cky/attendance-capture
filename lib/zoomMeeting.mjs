...
//
// Rather than reference the dialog by window name (it has none -- shows up
// as an unnamed window in the list, same as a couple of other Zoom popups),
// this searches every window's buttons by index for one whose name contains
// "computer audio" and clicks it directly in one atomic AppleScript call --
// sidesteps needing a name for the window at all, and avoids a race between
// "find the window" and "click in it" as two separate calls while the
// window list is still settling right after join.
      ...
      try
        ...
        repeat with b in btns
          try
            ...
            if bname contains "Computer Audio" or bname contains "computer audio" then
              click b

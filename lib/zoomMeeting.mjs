...
// Found live (2026-09-01): a Zoom auto-update rebranded the app to "Zoom
// Workplace" and, along with it, dropped the "Zoom Meeting..."-prefixed
// window entirely -- confirmed via a live accessibility dump during an
// actual class that the window list was instead: "zoom floating video
// window", an unnamed recording-notice popup, the audio-conference dialog,
// and "Zoom Workplace" (the single unified app window). Because isInMeeting()
// only ever matched MEETING_WINDOW_PREFIX, it silently returned false for
// the entire meeting -- joinMeeting() then always timed out waiting for it
// to flip true, even though the join itself had actually succeeded (a real
// person could see their own tile connected). This indicator window seems
// to persist regardless of whether Zoom Workplace is frontmost, so it's
// used as an OR alongside the legacy prefix rather than a replacement for
// it, in case a future/older Zoom build still uses the old window. NOTE:
// getMeetingWindowBounds() below still only searches MEETING_WINDOW_PREFIX
// -- it hasn't been fixed for this same rebrand yet, since that would
// require clicking around a live meeting's toolbar to re-measure offsets
// against the new "Zoom Workplace" window, which wasn't safe to do while a
// real class was in progress. Fix that (Participants-panel-open and
// leaveMeeting both depend on it) before trusting a fully unattended run
// end-to-end again.
const MEETING_INDICATOR_WINDOW_NAMES = ["zoom floating video window"];

function isMeetingWindowName(name) {
  const lower = name.toLowerCase();
  return lower.startsWith(MEETING_WINDOW_PREFIX.toLowerCase()) || MEETING_INDICATOR_WINDOW_NAMES.includes(lower);
}
...
export function isInMeeting() {
  ...
  return listWindowNames().some(isMeetingWindowName);
}
...
// Found live (2026-09-01), same Zoom Workplace rebrand as
// MEETING_INDICATOR_WINDOW_NAMES above: right after joining, there is no
// "Zoom Meeting"-prefixed window at all -- Zoom Workplace defaults to
// showing whatever app tab was last open (Chat, in the case that exposed
// this) with just the small "zoom floating video window" thumbnail panel
// floating over it, and no toolbar anywhere. getMeetingWindowBounds() then
// throws (confirmed: this was the exact "Couldn't confirm leaving the
// meeting" error seen in a real scheduled run's log), and every
// toolbar-based action (Participants, Leave) has nothing to click.
//
// Two things bring the classic "Zoom Meeting" window (and its toolbar)
// back, verified live against a real meeting: (1) the Window menu's "Zoom
// Meeting" item, which switches focus to it like any other open window,
// and (2) the Window menu's "Always Show Meeting Controls" toggle, which
// keeps the toolbar docked at the window's bottom permanently instead of
// only revealing on hover the way the pre-rebrand client did. Once both
// are done, the resulting window's bounds and toolbar-button offsets were
// re-measured live and turned out to match the pre-rebrand TOOLBAR/
// LEAVE_CONFIRM constants almost exactly, so those didn't need changing --
// only getting the window to exist and be addressable at all did.
// "Always Show Meeting Controls" is a persistent Zoom Workplace preference
// (its menu checkmark stuck after being set once), not per-meeting, but
// it's cheap to no-op if already on, so this checks first rather than
// assuming and blindly toggling it (which would turn it back OFF).
function isAlwaysShowMeetingControlsOn() {
  const script = `
tell application "System Events"
  tell process "zoom.us"
    return (AXMenuItemMarkChar of menu item "Always Show Meeting Controls" of menu 1 of menu bar item "Window" of menu bar 1) as string
  end tell
end tell`;
  try {
    return osascript(script).trim().length > 0;
  } catch {
    return false;
  }
}

export function ensureMeetingWindowVisible() {
  try {
    if (!isAlwaysShowMeetingControlsOn()) {
      osascript(`
tell application "System Events"
  tell process "zoom.us"
...
tell application "System Events"
  tell process "zoom.us"
    click menu item "${MEETING_WINDOW_PREFIX}" of menu 1 of menu bar item "Window" of menu bar 1
  end tell
end tell`);
  } catch {
    // Both menu items only exist once a meeting is actually underway --
    // harmless to fail here if called too early; callers needing the
    // window to exist (clickToolbarButton) will surface their own error.
  }
}
...
// Found live (2026-09-01): rejoining a SECOND time in the same session (the
// first join had already completed, then left) surfaced a different
// pre-join screen than the "Choose one of the audio conference options"
// modal above -- a "lobby" card titled after the meeting's own topic (e.g.
// "IIMI_MSDSM Batch 6 Term 1 Sessions"), showing "Host has joined. We've
// let them know you're here." with a muted-mic icon that needs a manual
// click before the join can proceed -- confirmed via its tooltip, "Click
// on the audio button to join to computer audio". Until this is clicked,
// isInMeeting() never goes true, so joinMeeting() below just times out
// exactly like the original "Zoom Meeting window doesn't exist yet" bug
// this file's other fixes addressed -- this is a genuinely different cause
// of the same symptom, not a re-occurrence of that one.
//
// CORRECTION (still 2026-09-01, same day as the fix above): that first
// attempt at this function did NOT actually work -- confirmed live when a
// real scheduled run hit this exact card again and still timed out, stuck
// on it for over a minute unattended. Dumped the card's full accessibility
// tree while it was stuck (window 2's `entire contents`) to find out why:
// the mic button isn't a direct child of the window OR exposed via `help`
// at all -- it's nested one level inside a `tab` element (itself a direct
// child of the window, description "Video render"), and its usable text is
// in `description` ("Audio, turn on, currently muted"), not `help` (empty
// for this button). `buttons of window i` only ever sees a window's direct
// children, so it silently never found this button, no matter how many
// times the poll loop below called it. Fixed by walking window -> child
// UI elements -> (if that child is a "tab" or "group") its buttons, one
// bounded level of nesting rather than a full recursive `entire contents`
// scan of every window (which would also crawl the huge "Zoom Workplace"
// chat UI every 2 seconds during the whole join wait), and matching on
// `description` instead of `help`.
//
// Second correction, same live debugging session: `tabs of w` / `class is
// tab` both fail outright -- "tab" is a reserved AppleScript constant (the
// tab whitespace character), not a plain identifier, so System Events reads
// `class is tab` as "class equals the tab character" and errors. Comparing
// `class of el as string` against the STRING "tab" inside a manual repeat
// over `UI elements of w` sidesteps the reserved word entirely, and this
// exact query was run live against the actual stuck window to confirm it
// returns the three expected buttons (Background & effects / Video / Audio)
// before being written back in here -- not just reasoned about.
//
// Third correction: matching on a loose "mic" substring clicked the
// unrelated "Test speaker and mic" button instead (it's a direct window
// child, checked before the nested Audio one) -- confirmed live by running
// the first version of this fix and watching it click the wrong thing on
// screen. Narrowed to "currently muted", which only the Audio button's
// description contains (the sibling Video button says "currently off").
//
// Fourth correction: even once the right button was finally being found,
// AppleScript's own `click b` (an AXPress action) did nothing at all --
// confirmed live by running it against the real stuck card and watching
// the screen not change. Same underlying reason the toolbar buttons
// elsewhere in this file (TOOLBAR/clickToolbarButton) need real synthetic
// mouse events via cliclick instead of AXPress: this Zoom build apparently
// doesn't wire every button's accessibility press action to its actual
// click handler. Switched to reading the button's bounds via AppleScript,
// then clicking its center with cliclick -- confirmed live this time by
// seeing the "Computer audio connected" toast actually appear on screen.
function findLobbyAudioButtonBounds() {
  const script = `
tell application "System Events"
  tell process "zoom.us"
    set n to count of windows
    repeat with i from 1 to n
      try
        set w to window i
        set topEls to UI elements of w
        repeat with el in topEls
          set elClass to "?"
          try
            set elClass to class of el as string
          end try
          if elClass is "tab" or elClass is "group" then
            try
              set btns to buttons of el
              repeat with b in btns
                set bdesc to "?"
                try
                  set bdesc to description of b as string
                end try
                if bdesc contains "currently muted" then
                  set {bx, byy} to position of b
                  set {bw, bh} to size of b
                  return (bx as string) & "," & (byy as string) & "," & (bw as string) & "," & (bh as string)
                end if
              end repeat
            end try
          end if
...
function dismissLobbyAudioPromptIfPresent() {
  const bounds = findLobbyAudioButtonBounds();
  if (!bounds) return false;
  const x = Math.round(bounds.x + bounds.width / 2);
  const y = Math.round(bounds.y + bounds.height / 2);
  execFileSync(CLICLICK, [`c:${x},${y}`]);
...
// The "This meeting is being recorded" notice -- a separate popup from
// both the audio dialog and the lobby card above, found live sitting on
// screen alongside the lobby card during the same debugging session. Its
// window has no name (same missing-window-name pattern as everything else
// in this file) but its "OK" button IS a direct child of the window with a
// real `description` ("OK") -- unlike the lobby/audio-dialog buttons, no
// nesting or coordinate fallback needed here. Scoped to a window that also
// contains the exact recording-notice text, not just any "OK" button
// anywhere, since "OK" alone is too generic a match to risk clicking blind
// in some unrelated dialog.
function dismissRecordingNoticeIfPresent() {
  const script = `
tell application "System Events"
  tell process "zoom.us"
    set n to count of windows
    repeat with i from 1 to n
      try
        ...
        try
          set texts to static texts of w
          repeat with t in texts
            set tval to "?"
            try
              set tval to value of t as string
            end try
            if tval contains "being recorded" then set hasNotice to true
          end repeat
        end try
        if hasNotice then
...
return "not found"`;
  try {
    return osascript(script).trim() === "clicked";
  } catch {
    return false;
  }
}
...
export async function joinMeeting(link, { timeoutSeconds = 45 } = {}) {
  ...
  while (Date.now() < deadline) {
    if (isInMeeting()) {
      ...
      // Both dialogs can appear independently of each other (and neither
      // reliably before the other), so every iteration checks both rather
      // than stopping after whichever is dismissed first.
      for (let i = 0; i < 3; i++) {
        const dismissedAudio = dismissAudioDialogIfPresent();
        const dismissedRecordingNotice = dismissRecordingNoticeIfPresent();
        if (!dismissedAudio && !dismissedRecordingNotice) break;
        await new Promise((r) => setTimeout(r, 300));
      }
      // Surfaces the "Zoom Meeting" window right away rather than waiting
      // for the first toolbar action to need it (clickToolbarButton calls
      // this too) -- cheap and idempotent, and gives the window a moment
      // to settle before lib/participants.mjs starts reading it.
      ensureMeetingWindowVisible();
      return;
    }
    // Unlike the audio dialog (only checked once we're already confirmed
    // in the meeting), the lobby prompt gates isInMeeting() ever going
    // true at all, so it has to be checked on every wait iteration, not
    // just after success.
    dismissLobbyAudioPromptIfPresent();
    dismissRecordingNoticeIfPresent();

...
//
// This card's mic button reports `missing value` for its name via System
// Events, same missing-accessible-title pattern as the audio dialog above,
// but its AXHelp (tooltip) attribute does carry the "audio" wording, so
// that's searched instead. NOT independently re-verified against a second
// live occurrence beyond the one that motivated this (it wasn't safe to
// deliberately reproduce this by rejoining a real class a third time) -- if
// a scheduled run still times out after this, check for this exact card
// with a fresh screenshot before assuming it's the original window-name
// issue again.
  ...
  tell process "zoom.us"
    ...
    repeat with i from 1 to n
      try
        ...
        repeat with b in btns
          try
            set bhelp to help of b as string
            if bhelp contains "audio" or bhelp contains "mic" then
              click b
              return "clicked:" & bhelp
            end if
          end try
        end repeat
      end try

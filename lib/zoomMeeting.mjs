...
export async function joinMeeting(link, { timeoutSeconds = 45 } = {}) {
  ...
  throw new Error(
    `Timed out after ${timeoutSeconds}s waiting to join the meeting. If Zoom is showing a pre-join preview dialog, either click "Join" by hand or disable "Always show this preview when joining" in Zoom's settings for unattended runs. This can also mean the screen was locked -- confirmed live (2026-08-29) that Zoom can genuinely join while locked, but isInMeeting() can't detect it (System Events can't see any window on a locked screen), so this looks identical to a real failure. A forced leave attempt still runs regardless (see mark-attendance.mjs's finally block) so the bot doesn't get stranded either way.`
  );
}
...
//
// `force: true` skips the isInMeeting() early-return -- for the one case
// where isInMeeting() itself can't be trusted: joinMeeting() timing out
// while the screen is locked. Found live (2026-08-29): the screen was
// locked when a scheduled run fired, and the user confirmed Zoom genuinely
// DID join -- but isInMeeting() (which reads window names via System
// Events) can't see anything while the screen is locked, a real macOS
// security boundary, not a bug to fix in this file. joinMeeting() throws
// its timeout error in that case, which meant mark-attendance.mjs never
// even got to set joinedByUs = true, so its own isInMeeting()-gated leave
// never ran either -- the bot could be left sitting in the meeting
// indefinitely, unmanaged, with no error surfaced. `force` lets a caller

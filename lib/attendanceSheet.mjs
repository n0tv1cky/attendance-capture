...
// to re-run, never overwrites an existing mark). Returns which row indices
// were actually written (not just a count), so callers logging per-student
// outcomes have one authoritative source for "was this cell blank before
// this run" instead of re-deriving the same blank-check separately.
export async function writePresent(sheets, spreadsheetId, tab, targetCol, rowIndicesToMark, presentValue) {
  const writtenRowIndices = rowIndicesToMark.filter((rowIndex) => !String(tab.students.find((s) => s.rowIndex === rowIndex).values[targetCol.index] ?? "").trim());

  if (writtenRowIndices.length === 0) return { written: 0, writtenRowIndices: [] };

  const data = writtenRowIndices.map((rowIndex) => ({
  ...
  return { written: data.length, writtenRowIndices };
}

// Appends the class date to a "Session N" header cell (e.g. "Session 6" ->
// "Session 6 (01-Sep-26)"), so a glance at the sheet shows which calendar
// date each column was for -- without having to cross-reference the
// schedule separately. Only touches the header when it's still the bare
// "Session N" text loadTab originally parsed (see its sessionPrefix match,
// which uses parseInt and so tolerates trailing text like this just fine) --
// never overwrites a header someone has already customized by hand, and
// never re-writes the same date twice on a re-run of the same session.
export async function ensureSessionHeaderDate(sheets, spreadsheetId, tab, targetCol, columnsConfig, date) {
  const bareHeader = `${columnsConfig.sessionPrefix}${targetCol.sessionNumber}`;
  const currentHeader = String(tab.header[targetCol.index] ?? "").trim();
  if (currentHeader !== bareHeader) return { updated: false };

  const day = String(date.getDate()).padStart(2, "0");
  const month = date.toLocaleDateString("en-US", { month: "short" });
  const year = String(date.getFullYear()).slice(-2);
  const newHeader = `${bareHeader} (${day}-${month}-${year})`;

  await sheets.spreadsheets.values.update({
    spreadsheetId,
    range: `'${tab.tabTitle}'!${columnLetter(targetCol.index)}1`,
    valueInputOption: "RAW",
    requestBody: { values: [[newHeader]] },
  });
  return { updated: true, newHeader };
}

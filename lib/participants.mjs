...
const ROLE_TAG_RE = /\((host|co-host)\)/i;
...
//
// Two independent signals mark a participant as "not a student, don't
// bother matching or flagging for review": Zoom's own (Host)/(Co-host) role
// tag, and a "Prof"/"Dr" title prefix -- covers a faculty member attending
// as a plain participant (no co-host tag) as well as one who is co-hosted.
// Neither is hardcoded to specific names, so both hold up next term with
// different faculty without maintenance. Routed to a separate "excluded"
// bucket upstream instead of "needs review" -- still visible in the tool's
// output, just correctly labeled as "not a student, nothing to do" rather
// than "look at this by hand."
export function parseParticipantList(rawText) {
  ...
  return lines
    .map((line) => {
      const roleMatch = line.match(ROLE_TAG_RE);
      const cleaned = line.replace(/\s*\([^)]*\)\s*/g, " ").trim();
      const excludeReason = roleMatch ? `${roleMatch[1].toLowerCase()} role` : TITLE_PREFIX_RE.test(cleaned) ? "title prefix (Prof/Dr)" : null;
      return { raw: line, cleaned, role: roleMatch ? roleMatch[1].toLowerCase() : null, excludeReason };
    })

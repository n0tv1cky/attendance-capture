...
//
// The (Host)/(Co-host) tag specifically is Zoom's own signal, not a guess:
// verified against a real class that every non-student participant present
// (an AV/recording studio account, a second studio account, and the faculty
// member) was tagged one of these two, while every genuine student wasn't.
// Rather than hardcode those specific names (which would need updating every
// term as faculty/AV setup changes), participants carrying this tag are
// flagged by role here and routed to a separate "excluded" bucket upstream
// instead of "needs review" -- still visible in the tool's output, just
// correctly labeled as "not a student, nothing to do" rather than "look at
// this by hand."
export function parseParticipantList(rawText) {
  ...
  return lines
    .map((line) => {
      ...
      return { raw: line, cleaned, role: roleMatch ? roleMatch[1].toLowerCase() : null };
    })
